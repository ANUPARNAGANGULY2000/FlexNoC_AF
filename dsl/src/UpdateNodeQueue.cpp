...
void update_node_from_Queue(std::vector<std::shared_ptr<Queue>>& queues){
   ...
   for(auto queue_index=0; queue_index!=queues.size(); ++queue_index){
	   ...
	   std::shared_ptr<Junc> node_address = mapping.node_data[node_name];//address of the node
...
void node_update_forward(std::vector<std::shared_ptr<Junc>>& nodes){
	...
	std::vector<std::shared_ptr<Junc>> connected_node;

	//if there is single node then update previous  node's value to next
	if(node_size == 1){
		std::shared_ptr<Junc> prev_node = nodes[0];
		std::shared_ptr<Junc> next_node = mapping.node_files[prev_node];
	  if(next_node){
	...
	//if there is multiple nodes then merge it and update that to next node.
	   ...
	   for(auto node_index=0; node_index<nodes.size(); ++node_index){
		   std::shared_ptr<Junc> node_address = nodes[node_index];
	 ...
	 std::shared_ptr<Junc> prev_node = nodes[0];// as both nodes destination is same so we are taking only one node
	 std::shared_ptr<Junc> next_node = node_files[prev_node];
